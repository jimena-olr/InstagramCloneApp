...
export async function getPostsForUser(viewerId) {
  ...
  try {
    // Step 1: Get ranked post IDs for the viewer
    ...
    // Step 2: Fetch full post info with like status and counts
    const [posts] = await db.send_sql(
      `SELECT 
         ...
         COUNT(pl_all.user_id)    AS likeCount,
         (pl_viewer.user_id IS NOT NULL) AS liked
       FROM posts p
       JOIN users u ON p.author = u.user_id
       LEFT JOIN post_likes pl_all ON pl_all.post_id = p.post_id
       LEFT JOIN post_likes pl_viewer
              ON pl_viewer.post_id = p.post_id AND pl_viewer.user_id = ?
       WHERE p.post_id IN (?)
       GROUP BY p.post_id`,
      [viewerId, postIds]
    );

    // Step 3: Reorder results to match feed rank order
